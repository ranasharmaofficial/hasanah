										...
										<div class="row mb-3">
											<table class="table table-bordered">
												<tbody>
                                                    @php
                                                        $ldate = date('Y-m-d');
                                                        $datedays = strtotime($project_details->no_of_days)-strtotime($ldate);
                                                        $datediff = (round($datedays / 86400));
                                                    @endphp
													<tr>
														<td>Contractor&nbsp;Name</td>
														<td class="text-right">{{$contractor_details->name}} ({{$contractor_user_id}})</td>
													</tr>
													<tr>
														<td>Company</td>
														<td class="text-right">{{$company_details}}</td>
													</tr>
													<tr>
														<td>Category</td>
														<td class="text-right">{{$project_details->project_category}}</td>
													</tr>
													<tr>
														<td>Project&nbsp;Name</td>
														<td class="text-right">{{$project_details->project_name}}</td>
													</tr>
													<tr>
														<td>Project&nbsp;Number</td>
														<td class="text-right">{{$project_details->project_number}}</td>
													</tr>
                                                    <tr>
                                                    ...
                                                    <tr>
														...
														<td class="text-right text-primary">{{$project_details->currency}}&nbsp;{{$project_details->amount}}</td>
													</tr>
                                                    <tr>
														<td>Days&nbsp;to&nbsp;Go</td>
														<td>
                                                            <p class="bg-success text-white p-1 rounded shadow"><strong>Date&nbsp;From:</strong>&nbsp;{{$project_details->created_at->format('d-m-Y')}}</p>
                                                            <p class="bg-danger text-white p-1 rounded shadow"><strong>Days to left:</strong>&nbsp;{{$datediff}}</p>
                                                        </td>
													</tr>
                                                    @if($project_details->action=='3')
                                                    <form action="{{route('employee.project-approve')}}" method="post" enctype="multipart/form-data"> 
                                                        @csrf

                                                        <input type="hidden" name="contractorid" value="{{$contractor_user_id}}" required />
                                                        <input type="hidden" name="projectid" value="{{$project_details->project_id}}" required />
                                                        <input type="hidden" name="companyid" value="{{$project_details->company_id}}" required />
                                                        ...
                                                        <tr style="background: #DCFFF4; border: none;">
                                                            <td style="border: none;">
                                                                <input type="tel" class="form-control" name="amount" value="{{$project_details->distribute_amount}}" required placeholder="Total Amount" readonly>
